...
@section('content')
    <div class="overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl">
        <div class="relative flex min-h-64 items-end overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 p-6 sm:p-10">
            @if ($course->image_path)<img class="absolute inset-0 h-full w-full object-cover opacity-40" src="{{ Storage::disk('public')->url($course->image_path) }}" alt="">@endif
            <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 to-transparent"></div>
            <div class="relative">
                <span class="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-sm font-semibold text-indigo-100"><i class="bi bi-play-circle"></i>{{ $course->lessons->count() }} lecciones</span>
                <h1 class="mt-4 text-3xl font-black tracking-tight sm:text-5xl">{{ $course->title }}</h1>
                <p class="mt-3 max-w-2xl text-slate-200">{{ $course->short_description }}</p>
            </div>
        </div>
    </div>
                ...
                <div class="mt-4 space-y-3">
                    @foreach ($course->lessons as $lesson)
                        <div class="erp-card flex items-center gap-4 p-4 transition hover:border-indigo-200">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl {{ $enrolled || $lesson->is_preview ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400' }}"><i class="bi {{ $enrolled || $lesson->is_preview ? 'bi-play-fill' : 'bi-lock-fill' }}"></i></span>
                            <div class="min-w-0 flex-1">
                                <strong class="block truncate">{{ $loop->iteration }}. {{ $lesson->title }}</strong>
                                <p class="mt-1 truncate text-sm text-slate-500">{{ $lesson->description ?: 'Lección en video' }}</p>
                            </div>
                            @if ($enrolled || $lesson->is_preview)
                                <a class="shrink-0 text-sm font-bold text-indigo-600 hover:text-indigo-800" href="{{ route('lessons.video', [$course, $lesson]) }}">Ver <i class="bi bi-arrow-right"></i></a>
                            @else
                                <span class="shrink-0 text-xs font-semibold text-slate-400">Bloqueado</span>
                            @endif
                        </div>
                    @endforeach
                </div>
