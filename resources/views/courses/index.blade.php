...
@section('content')
    ...
    <div class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        @foreach ($courses as $course)
            <article class="group erp-card overflow-hidden transition hover:-translate-y-1 hover:shadow-lg">
                <a href="{{ route('courses.show', $course) }}" class="relative block h-48 overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600">
                ...
                <div class="p-5">
                    <div class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-indigo-600"><i class="bi bi-play-btn"></i>{{ $course->lessons_count }} lecciones</div>
                    <h2 class="mt-3 text-xl font-bold text-slate-900">{{ $course->title }}</h2>
                    <p class="mt-2 line-clamp-2 text-sm leading-6 text-slate-500">{{ $course->short_description }}</p>
                    <a class="mt-5 inline-flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-800" href="{{ route('courses.show', $course) }}">Ver curso <i class="bi bi-arrow-right"></i></a>
                </div>
            </article>
        @endforeach
    </div>
