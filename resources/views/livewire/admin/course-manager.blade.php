    ...
    <div class="erp-card overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[700px] text-left">
                    ...
                    <tr class="transition hover:bg-slate-50">
                        <td class="px-6 py-5">
                            <div class="flex items-center gap-3">
                                <span class="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 text-lg text-white">
                                    <i class="bi bi-play-btn"></i>
                                </span>
                                <div>
                                    <strong class="block">{{ $course->title }}</strong>
                                    <small class="text-xs text-slate-500">{{ $course->slug }}</small>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5">
                            <span @class([
                                ...
                                'bg-emerald-100 text-emerald-700' => $course->is_published,
                                'bg-slate-100 text-slate-600' => ! $course->is_published,
                            ])>
                        ...
                        <td class="px-6 py-5 text-right">
                            ...
                            <button
                                class="mr-3 text-sm font-semibold text-slate-500 hover:text-indigo-600"
                                wire:click="toggle({{ $course->id }})"
                                wire:confirm="{{ $course->is_published ? '¿Despublicar este curso?' : '¿Publicar este curso?' }}"
                            >
                            ...
                            <button
                                ...
                                wire:confirm="¿Eliminar este curso? Esta acción no se puede deshacer."
                            >
                ...
                @empty
                    <tr>
                        <td class="p-10 text-center text-slate-500" colspan="4">
                            <i class="bi bi-journal-x mr-2"></i>
                            {{ $search ? 'No encontramos cursos con ese criterio.' : 'Todavía no hay cursos.' }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
