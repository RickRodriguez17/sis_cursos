...
<html lang="es">
    ...
    <body class="bg-slate-100 font-sans text-slate-900 antialiased">
        <div x-data="{ sidebarOpen: false }" class="min-h-screen">
            ...
            ></div>
            <aside
                class="fixed inset-y-0 left-0 z-50 flex w-72 -translate-x-full flex-col bg-slate-900 text-white shadow-xl transition-transform duration-300 lg:translate-x-0"
            ...
            >
                <div class="flex h-20 shrink-0 items-center justify-between border-b border-slate-800 px-6">
                    <a href="{{ route('admin.index') }}" class="flex items-center gap-3">
                        <span class="erp-icon bg-indigo-600 text-white">
                            <i class="bi bi-mortarboard-fill"></i>
                        </span>
                        <span>
                            <strong class="block tracking-tight">Aula Viva</strong>
                            <small class="text-xs text-slate-400">Panel administrativo</small>
                        </span>
                    </a>
                ...
                <nav class="flex-1 space-y-2 overflow-y-auto px-3 py-5">
                    <a
                        ...
                        @class([
                            ...
                            'bg-indigo-600 text-white' => request()->routeIs('admin.index'),
                            'text-slate-300 hover:bg-slate-800 hover:text-white' => ! request()->routeIs('admin.index'),
                        ])
                    >
                    ...
                    <x-sidebar-group title="Operaciones" icon="bi-receipt" :active="request()->routeIs('admin.orders', 'admin.enrollments')">
                        <x-sidebar-link
                ...
                <div class="border-t border-slate-800 p-4">
                    <a
                        ...
                        class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white"
                    >
                    ...
                    <form class="mt-2" method="post" action="{{ route('logout') }}">
                        @csrf
                        <button class="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-rose-300 hover:bg-rose-950/40">
                ...
                <header class="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200 bg-white/95 px-4 shadow-sm backdrop-blur sm:px-6">
                    ...
                    <div class="flex items-center gap-3">
                        <span class="hidden text-right sm:block">
                            ...
                            <small class="text-xs text-slate-500">{{ auth()->user()->email }}</small>
                        </span>
        ...
        @if (session('error'))
            <script>
                window.addEventListener('load', () => window.erpAlert({
                    icon: 'error',
