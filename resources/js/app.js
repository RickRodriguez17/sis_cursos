...
document.addEventListener('click', (event) => {
    ...
    const match = expression?.match(/^\s*(delete|cancel|annul|anular|toggle)\s*(?:\((.*)\))?\s*$/i);

    if (! match || element.dataset.confirmationHandled === 'true' || ! element.hasAttribute('wire:confirm')) {
        return;
    }
    ...
    if (instance) {
        window.confirmLivewireAction(instance, match[1], params, element.getAttribute('wire:confirm'))
            .finally(() => {
